/**
 * 🔄 REPROCESS BATCH 4 - Próximos 20 produtos
 */
...
const REPROCESSED_DIR = 'debug/reprocessed-batch-4';
...
async function reprocessBatch4(): Promise<void> {
  console.log('🔄 BATCH 4 - PRÓXIMOS 20 PRODUTOS\n');
  ...
  console.log('='.repeat(80));
  console.log(`📊 BATCH 4: ${approved + alreadyClean}/${productsToProcess.length} aprovados (${((approved + alreadyClean) / productsToProcess.length * 100).toFixed(1)}%)\n`);

  await fs.writeFile('debug/reprocess-batch-4-report.json', JSON.stringify({
  ...
  console.log('💾 debug/reprocess-batch-4-report.json');
  console.log('📁 debug/reprocessed-batch-4/\n');
}

reprocessBatch4().catch(console.error);
