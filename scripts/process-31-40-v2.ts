...
const OUTPUT_DIR = path.join(process.cwd(), 'debug', 'qwen', 'processed-31-40-v2');
...
async function main() {
  console.log('🚀 PROCESSAMENTO DOS PRODUTOS 31-40 - V2 COM COR PRESERVADA\n');
  ...
  const allImages = fs.readdirSync(SOURCE_DIR)
    ...
    .sort()
    .slice(30, 40); // Products 31-40

  console.log(`📸 Total de imagens para processar: ${allImages.length}\n`);
  console.log(`🔢 Processando produtos 31-40\n`);
  ...
  console.log('\n' + '='.repeat(80));
  console.log('📊 RESUMO DO PROCESSAMENTO (PRODUTOS 31-40 V2)\n');
  ...
  console.log('\n📊 PROGRESSO GERAL:');
  console.log(`   Total processado até agora: 40/476 produtos (8%)`);
  console.log(`   Faltam: 436 produtos`);
