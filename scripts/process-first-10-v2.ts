...
const OUTPUT_DIR = path.join(process.cwd(), 'debug', 'qwen', 'processed-10-v2');
...
async function editImageWithQwen(imagePath: string): Promise<string> {
  ...
  // Use qwen-edit service (now with box removal instructions)
  ...
  console.log('   ✅ Qwen editou a imagem (incluindo caixa)');
...
async function applySmartBlur(
  ...
  // Apply INTENSE blur (30 instead of 15)
...
async function processImage(imagePath: string, filename: string): Promise<ProcessResult> {
  ...
  try {
    // Step 1: Qwen Edit (now with box removal)
    ...
    // Step 3: Smart Blur (if needed) - now more intense
...
async function main() {
  console.log('🚀 REPROCESSAMENTO DOS PRIMEIROS 10 PRODUTOS (V2 - MELHORADO)\n');
  console.log(`📂 Origem: ${SOURCE_DIR}`);
  console.log(`📁 Destino: ${OUTPUT_DIR}\n`);
  console.log('✨ MELHORIAS:\n');
  console.log('   1️⃣  Qwen agora remove logos/textos da CAIXA do produto também');
  console.log('   2️⃣  Smart blur MAIS INTENSO (blur=30 ao invés de 15)\n');
  ...
  // Summary
  console.log('\n' + '='.repeat(80));
  console.log('📊 RESUMO DO REPROCESSAMENTO (V2)\n');
  ...
  console.log(`   └─ Com smart blur INTENSO:  ${withBlur}`);
  console.log(`❌ Falhas:                     ${failed}`);

  console.log('\n📁 Imagens salvas em:', OUTPUT_DIR);
  console.log('\n💡 Compare com a versão anterior em: debug/qwen/processed-10/');
